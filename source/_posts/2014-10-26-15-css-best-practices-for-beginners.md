...
# Code styling, conventions and best practices for beginners
- or -
# 15 CSS Best practices for beginners
...
The bad example in this case will still work. But it's pretty much confusing! So always use spaces properly.
...
So the only reason because you should just use classes, is because of specificity. You can almost completely forget about it if you use classes.


### Choose your style and be consistent

It doesn't really matter whether you use `.camelCase`, `.snake_case` or `.Hyphens-everywhere` for your class names. The important thing is that you choose a style and you stick with it. Conventions can be extremely powerful: take advantage of them.
...
Always namespace your selector, or bad things will happen. For example, you should never directly add some rules directly to an <article> or a <section>. Instead, wrap them in a class. They are bad because they will apply to all elements of that type in the page. And they will probably be overwritten by a class, which has way more CSS specificity. Last but not least, class selectors are more performant than element selectors.
...
The style and the markup shouldn’t be tightly coupled, because we lost in scalability. For example, consider this HTML and CSS:
